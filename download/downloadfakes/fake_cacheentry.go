...
type FakeCacheEntry struct {
	...
	StoreStub        func(contents io.ReadCloser, etag string, checksum string) error
...
func (fake *FakeCacheEntry) Store(contents io.ReadCloser, etag string, checksum string) error {
	...
	fake.storeArgsForCall = append(fake.storeArgsForCall, struct {
		...
		etag     string
		checksum string
	}{contents, etag, checksum})
	fake.recordInvocation("Store", []interface{}{contents, etag, checksum})
	fake.storeMutex.Unlock()
	if fake.StoreStub != nil {
		return fake.StoreStub(contents, etag, checksum)
	}
	if specificReturn {
		return ret.result1
	}
...
func (fake *FakeCacheEntry) StoreArgsForCall(i int) (io.ReadCloser, string, string) {
	fake.storeMutex.RLock()
	defer fake.storeMutex.RUnlock()
	return fake.storeArgsForCall[i].contents, fake.storeArgsForCall[i].etag, fake.storeArgsForCall[i].checksum
}
